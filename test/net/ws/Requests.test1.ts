...
describe("web socket requests", () => {
        ...
        it("should return account relative history", (done: (arg?: any) => void) => {
            rxWs.request(new GetRelativeAccountHistory(ChainObject.parse("1.2.35"), 10, 20))
                .subscribe((value) => value.should.all.instanceOf(OperationHistory), (error) => done(error), () => done());
        });
        ...
        it("should return miners by name lookup", (done: (arg?: any) => void) => {
            rxWs.request(new LookupMinerAccounts(""))
                .subscribe((value) => undefined, (error) => done(error), () => done());
        });

        it("should request api access, has to login before", (done: (arg?: any) => void) => {
            rxWs.request(new Login()).pipe(flatMap(() =>
